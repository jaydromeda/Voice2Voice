...
# FIX: PyTorch 2.6 compatibility - allow fairseq classes
...
async def infer_audio(
    ...
    """
    Inference endpoint for file-based RVC conversion.
    Does not require /start - loads model on demand.
    """
    input_path = None
    output_path = None
    
    try:
        logger.info(f"Received inference request: f0up_key={f0up_key}, f0method={f0method}")
        
        # Check if configuration exists
        if not audio_api.gui_config.pth_path:
            raise HTTPException(
                status_code=400, 
                detail="Model not configured. Please call /config endpoint first with pth_path and index_path."
            )
        
        if not os.path.exists(audio_api.gui_config.pth_path):
            raise HTTPException(
                status_code=400,
                detail=f"Model file not found: {audio_api.gui_config.pth_path}"
            )
        
        if not os.path.exists(audio_api.gui_config.index_path):
            raise HTTPException(
                status_code=400,
                detail=f"Index file not found: {audio_api.gui_config.index_path}"
            )
        ...
        # Initialize one-time RVC inference
        logger.info("Initializing RVC for inference...")
        from tools import rvc_for_realtime
        
        # Create a temporary RVC instance just for this inference
        # Parameters match the original start_vc() method
        temp_rvc = rvc_for_realtime.RVC(
            f0up_key,  # First positional parameter is pitch
            audio_api.gui_config.pth_path,
            audio_api.gui_config.index_path,
            index_rate,
            audio_api.gui_config.n_cpu,
            audio_api.inp_q,
            audio_api.opt_q,
            audio_api.config,
            None  # rvc parameter (for reusing existing model)
        )
        
        logger.info(f"RVC initialized with target_sr={temp_rvc.tgt_sr}")
        
        # Convert audio to torch tensor
        audio_tensor = torch.from_numpy(audio).to(audio_api.config.device)
        logger.info(f"Audio tensor: shape={audio_tensor.shape}, device={audio_tensor.device}")
        
        # Prepare for inference
        block_frame_16k = len(audio)
        skip_head = 0
        return_length = len(audio) // 160
        
        logger.info(f"Running inference: block_frame_16k={block_frame_16k}, return_length={return_length}")
        
        # Run inference
        output_audio = temp_rvc.infer(
            audio_tensor,
        ...
        output_numpy = output_audio.cpu().numpy()
        
        # Ensure output is in valid range
        output_numpy = np.clip(output_numpy, -1.0, 1.0)
        
        sf.write(output_path, output_numpy, temp_rvc.tgt_sr)
        logger.info(f"Output saved to: {output_path} (sr={temp_rvc.tgt_sr})")
        ...
        logger.error(f"Inference failed: {type(e).__name__}: {str(e)}")
        import traceback
        ...
        # Cleanup temp files (but not output_path as it's being returned)
