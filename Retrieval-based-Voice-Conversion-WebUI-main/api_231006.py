...
# Add this route to api_231006.py
@app.post("/convert_file")
async def convert_file(
    file: UploadFile = File(...),
    model_pitch: int = Form(0),
):
    """
    Accepts an uploaded file, runs RVC conversion (using the pth/index configured via /config),
    and returns a WAV file (16-bit PCM).
    """
    try:
        # Check config has been set
        pth = audio_api.gui_config.pth_path
        idx = audio_api.gui_config.index_path
        if not pth or not idx or pth.strip() == "" or idx.strip() == "":
            raise HTTPException(status_code=400, detail="Model (.pth) and index must be configured first via /config")

        # Lazy-load RVC (create audio_api.rvc if None)
        if audio_api.rvc is None:
            # Use same constructor as start_vc (arguments may vary by RVC version)
            audio_api.rvc = rvc_for_realtime.RVC(
                model_pitch,
                pth,
                idx,
                audio_api.gui_config.index_rate,
                0,
                0,
                0,
                audio_api.config,
            ...
            audio_api.gui_config.samplerate = audio_api.rvc.tgt_sr

        # Read uploaded bytes
        data = await file.read()
        # load audio as mono float32 at model sample rate
        wav, sr = librosa.load(io.BytesIO(data), sr=audio_api.gui_config.samplerate, mono=True)
        # rvc_for_realtime expects a 1-D numpy array in float32
        wav = wav.astype("float32")

        # Prepare arguments for rvc.infer -- this mirrors the infer call in audio_callback
        # We need a block-frame length for infer; we'll run whole audio as a single inference
        # many RVC versions accept: (input_wav_res, input_wav_res[-f0frame:], block_frame_16k, valid_rate, pitch, pitchf, f0method)
        # For simplicity: we call the RVC's simple API if it exposes a `single` or `infer` method that accepts wav and returns wav.
        # Fallback: try rvc.infer_simple / rvc.infer if available:

        # Try common simple API:
        if hasattr(audio_api.rvc, "infer_audio"):
            out = audio_api.rvc.infer_audio(wav)
        elif hasattr(audio_api.rvc, "infer"):
            # Some versions expect (waveform, f0_wav, block_frame_16k, valid_rate, pitch, pitchf, f0method)
            # We'll compute minimal dummy arguments:
            block_frame_16k = max(160, int(0.5 * 160))  # conservative placeholder; rvc impl will vary
            valid_rate = 1.0
            # create dummy pitch arrays
            pitch = np.zeros(int(len(wav) / 160) + 1, dtype="int32")
            pitchf = np.zeros_like(pitch, dtype="float64")
            # f0method fallback
            f0method = audio_api.gui_config.f0method if hasattr(audio_api.gui_config, 'f0method') else "rmvpe"
            # If infer expects frames, convert wav to 16k resampled if needed:
            # Some implementations expect wav already resampled to 16k; adjust if required by your RVC version.
            out = audio_api.rvc.infer(wav, wav, block_frame_16k, valid_rate, pitch, pitchf, f0method)
        else:
            raise HTTPException(status_code=500, detail="RVC model API not compatible - no suitable infer method found")

        # Ensure numpy array and float32 -> prepare WAV bytes
        if isinstance(out, torch.Tensor):
            out = out.cpu().numpy()
        if out.dtype != np.float32:
            out = out.astype("float32")

        # Write to bytes buffer as 16-bit PCM WAV at model sample rate
        buf = io.BytesIO()
        sf.write(buf, out, audio_api.gui_config.samplerate, subtype="PCM_16")
        buf.seek(0)
